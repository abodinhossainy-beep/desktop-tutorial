...
function BedroomSet({ color }) {
  const material = useMemo(() => ({ color, roughness: 0.34, metalness: 0.02 }), [color]);
  return (
    <group position={[0, -0.15, 0]}>
      ...
      <mesh position={[0, 0.88, 0.1]} castShadow><boxGeometry args={[3.45, 0.48, 3.8]} /><meshStandardMaterial color="#d6c7b5" roughness={0.75} /></mesh>
      {[-2.55, 2.55].map((x) => (
...
function App() {
  ...
  const price = selectedProduct.price + finish.extra + size.extra;
  const formattedPrice = price.toLocaleString('fa-IR');

  const selectProduct = (product) => {
    setSelectedProduct(product);
    const matchingFinish = finishes.find((f) => f.value === product.color);
    if (matchingFinish) setFinish(matchingFinish);
  ...
  return (
    <main className="app">
      <header className="header">
        ...
        <button className="cart">سبد خرید <span>{cartCount.toLocaleString('fa-IR')}</span></button>
      </header>
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><button className="primary" onClick={() => document.getElementById('custom')?.scrollIntoView({ behavior: 'smooth' })}>شروع سفارشی‌سازی <span>←</span></button><button className="ghost" onClick={() => document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' })}>مشاهده مجموعه</button></div>
          <div className="trust"><span>✓</span> تولید مستقیم در کارگاه <span>·</span> ضمانت کیفیت چوب</div>
        </div>
        <div className="scene-card">
          <div className="scene-badge">{selectedProduct.name} · مدل سه‌بعدی</div>
          ...
          <Canvas shadows camera={{ position: [6.8, 4.7, 7.4], fov: 42 }}>
            ...
            <directionalLight position={[5, 8, 5]} intensity={3.2} castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048} />
            <Environment preset="apartment" />
            <BedroomSet color={finish.value} />
            <ContactShadows position={[0, -0.05, 0]} opacity={0.35} scale={12} blur={2.5} />
            <OrbitControls enablePan={false} minDistance={5} maxDistance={12} minPolarAngle={0.55} maxPolarAngle={1.45} />
          </Canvas>
          <div className="camera-tools"><button className={cameraMode === 'front' ? 'active' : ''} onClick={() => setCameraMode('front')}>جلو</button><button className={cameraMode === 'side' ? 'active' : ''} onClick={() => setCameraMode('side')}>کنار</button><button className={cameraMode === 'top' ? 'active' : ''} onClick={() => setCameraMode('top')}>بالا</button></div>
      ...
      <section className="configurator" id="custom">
        ...
        <div className="config-row">
          <div><label>رنگ چوب</label><div className="swatches">{finishes.map((f) => <button key={f.name} className={finish.name === f.name ? 'swatch active' : 'swatch'} style={{ background: f.value }} title={f.name} onClick={() => setFinish(f)} />)}</div><small>{finish.name}</small></div>
          <div><label>ابعاد تخت</label><div className="sizes">{sizes.map((s) => <button className={size.label === s.label ? 'selected' : ''} key={s.label} onClick={() => setSize(s)}>{s.label}</button>)}</div></div>
          <div className="price"><small>قیمت نهایی مدل</small><strong>{formattedPrice} <b>تومان</b></strong><button onClick={() => setCartCount((c) => c + 1)}>افزودن به سبد خرید</button></div>
        </div>
      </section>
      ...
      <section className="process"><p className="eyebrow">HOW IT WORKS</p><h2>از انتخاب تا تحویل</h2><div className="process-grid"><div><b>۰۱</b><h3>انتخاب مدل</h3><p>مدل سه‌بعدی مورد علاقه خود را انتخاب کنید.</p></div><div><b>۰۲</b><h3>شخصی‌سازی</h3><p>رنگ، ابعاد و جزئیات را متناسب با اتاق خود تغییر دهید.</p></div><div><b>۰۳</b><h3>ثبت سفارش</h3><p>قیمت نهایی را ببینید و سفارش خود را ثبت کنید.</p></div><div><b>۰۴</b><h3>ساخت و ارسال</h3><p>در کارگاه ساخته شده و با بسته‌بندی امن ارسال می‌شود.</p></div></div></section>
